...
export const updateTask = async (req, res, next) => {
  try {
    ...
    if (!task) return next(err({ status: 404, message: 'Task not found' }));
    if (task.user.toString() !== req.user.id) return next(err({ status: 401, message: "It's not your todo." }));
...
export const getCurrentProfileTasks = async (req, res, next) => {
  try {
